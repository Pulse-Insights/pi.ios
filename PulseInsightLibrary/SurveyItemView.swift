...
//  PulseInsights
//
//  Created by long shen on 2024/5/16.
...
open class SurveyItemView: UIView {
    ...
    func displayCloseButton(_ display: Bool) {
        closeButton.isHidden = !display
    }
    ...
    fileprivate func changeToSurveyItem(_ surveyItem: SurveyTicket) {
        ...
        if strItemType == "single_choice_question" {
            ...
            chooseContentView.setupItem(true, surveyContent: surveyItem) { (strSelectedID, strNextID) -> Void in
                self.delegate?.onAnswerSelected(self, didUpdateAnswer: strSelectedID)
            }
            ...
            freeTextView.onChangeListener.delegate(on: self) { (_, void) ->
                Void in
                self.delegate?.onAnswerSelected(self, didUpdateAnswer: self.freeTextView.getInputString())
            }
...
protocol SurveyItemViewDelegate {
